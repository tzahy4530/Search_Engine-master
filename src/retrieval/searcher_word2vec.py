...
import numpy as np
from src.preprocess.stemmer import Stemmer
...
class Searcher:
    ...
    def search(self, query, k=None):
        ...
        # we using word2vec trained module, so whe looking for all words in distance 0.6 from all query tokens
        # then add it to query
        try:
            candidates = self._model.most_similar(positive=query_as_list)
            g = 0
            for i in range(len(candidates)):
                if candidates[i][0] in self.inverted_index:
                    if candidates[i][1] > 0.65:
                        query_as_list.append(self.stemmer.stem_term(candidates[i][0]))
                        g += 1
                        # break
                        if g > 20:
                            break
